...
class CLIP_TWO(Finetune_IL):
    def __init__(self, logger, config):
        ...
        if config.backbone == "clip_vit_b_16_224":
            ...
            # self._pretrained_weights_path = "/home/zhangwentao/Dataset1/Huggingface_pretrained_weights/clip_vit_base_patch16"
        ...
        if self._m_desp:
            ...
            elif config.dataset == 'cub200_i2t':
                desp_json = 'datasets/cub200_one_prompt.json' 
        ...
        for i in range(len(id_class_desp)):
            self._id_text_tokens.update({i : self._clip_process.tokenizer(id_class_desp[i],return_tensors='pt', padding=True)})
        ...
        self._network = clip_two(self._logger, self._pretrained_weights_path)
    ...
    def incremental_train(self):
        ...
        self._network = self._network.cuda()
        self._logger.info('Total params: {}'.format(count_parameters(self._network)))
    ...
    def _epoch_train(self, model, train_loader,  optimizer, scheduler, text_tokens=None, task_begin=None, task_end=None, task_id=None):
        ...
        for _, inputs, targets in train_loader:
            ...
            # all_targets = targets
            with autocast():
                image_features_all, text_features = model(inputs, text_tokens["input_ids"][cur_pre_classes].cuda(), text_tokens["attention_mask"][cur_pre_classes].cuda())  # forward(self, image_inputs, input_ids, attention_mask)
